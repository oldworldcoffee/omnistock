...
export default function OnlineShoppingList({ carts, vendors, onClearCart }) {
  ...
  const allMarked = (vendorId, cart) => {
    return cart.every(item => markedItems[`${vendorId}_${item.item_id}`]);
  };
    ...
    <div className="space-y-6">
          ...
          <div key={vendorId} className="border border-border rounded-lg overflow-hidden">
            <div className="bg-muted/50 px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  size="sm"
                  variant={vendorComplete ? "default" : "outline"}
                  onClick={() => handleVendorComplete(vendorId, cart)}
                >
            ...
            <div className="divide-y divide-border">
              {cart.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`p-4 flex items-center gap-4 hover:bg-muted/30 ${isMarked ? 'bg-success/5' : ''}`}
                  >
                      ...
                      {imageUrl ? (
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.target.parentElement.innerHTML = '<span class="text-xs text-muted-foreground">No image</span>';
                          }}
                        />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className={`font-medium ${isMarked ? 'line-through text-muted-foreground' : ''}`}>
                          {item.item_name}
                        </h4>
                        {isMarked && (
                      ...
                      <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                        <span>Qty: <span className="font-medium text-foreground">{item.qty}</span> {item.unit_of_measure}</span>
      ...
      {Object.keys(carts).length === 0 && (
        <div className="text-center py-12">
          <div className="w-12 h-12 mx-auto text-muted-foreground mb-3 flex items-center justify-center">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
            </svg>
          </div>
          <p className="text-muted-foreground">No items in your online shopping list</p>
        </div>
      )}
    </div>
  );
